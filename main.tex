...
\centering
\begin{tikzpicture}
\node[inner sep=0] (ui) {\includegraphics[width=\linewidth]{figures/ui-annotated.png}};
% Offsets are the measured 1920x1080 pixel positions of each target scaled by
% 174.26mm / 1920px, so a reshoot only needs the pixel numbers re-read.
% Lock-stage geometry: travel markers (258,626); dock ring (738,792);
% predicate readout (1485,800); ranked list (1518,550); amp curve (1513,973).
\node[callout] at ([xshift=23.4mm,yshift=-56.8mm]ui.north west) {1};
\node[callout] at ([xshift=67.0mm,yshift=-71.9mm]ui.north west) {2};
\node[callout] at ([xshift=136.6mm,yshift=-74.2mm]ui.north west) {3};
\node[callout] at ([xshift=137.8mm,yshift=-49.9mm]ui.north west) {4};
\node[callout] at ([xshift=137.3mm,yshift=-88.3mm]ui.north west) {5};
\end{tikzpicture}
...
\textbf{Step 5 (overlap).} \emph{Does:} read the inspector curve and the tree-prefix dock. \emph{Learns:} ranking never uses labels. The curve is post-hoc association, $P(\mathrm{same\ class}\mid\mathrm{shared\ leaves})$, with pair counts because the high-overlap tail is thin. On this seed the global exact top-$8$ purity rises from $0$ with one tree to $0.875$ with $200$; the pooled tree-prefix curve on the sample barely moves, which is why the booth reports the measured overlap curve rather than a vote story.
